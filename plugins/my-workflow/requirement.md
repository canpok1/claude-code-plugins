...
│       │   └── draft/, todo/, in_progress/, pending/, review/, done/
...
3. in_progress: TLが検知して移動。このフェーズで `tasks/todo` にタスクが生成される。
    - pending: PdMへの質問が必要な場合にTLが移動。質問解決後に todo に戻す。
...
5. done: TLがマージ完了後に移動。マージ成功を意味する。
※ レビューで問題が見つかった場合は、レビュー指摘をタスクファイルに追記し `todo` に戻す。Devが同じブランチで再度作業する。
...
- **仕様書** : `D{yyyyMMddHHmmssSSS}_{Title}.md` (例: `D20260208143000123_Login-System.md` )
- **タスク** : `D{design-ID}_T{yyyyMMddHHmmssSSS}_{Assignee}_{Title}.md` (例: `D20260208143000123_T20260208150000456_dev-a_Auth-Logic.md` )
- **提案** : `P{yyyyMMddHHmmssSSS}_{Title}.md` (例: `P20260208160000789_Update-Skill.md` )
- **質問** : `Q{yyyyMMddHHmmssSSS}_{Title}.md` (例: `Q20260208170000321_Auth-Scope.md` )

IDはタイムスタンプ（`yyyyMMddHHmmssSSS`形式、ミリ秒を含む）で採番する。ファイル作成時点のタイムスタンプを使用することで、複数エージェントの同時作成時のID衝突を回避する。
...
1. 引数で下記を受け取る
    - プロジェクトルートのパス
    - エージェント名
2. membersディレクトリにファイル配置（多重起動チェック）
    - `PO_*.md` が既に存在する場合は終了（POは1名のみ）
3. ユーザー向けメッセージを表示
...
5. 回答済みの質問ファイルを見つけたら下記を実施
    1. 回答を確認してタスクを再開
        ...
        - 追加質問がなくタスク再開できるなら質問ファイルをresolvedディレクトリに移動する
            - 質問の元になった仕様書・タスクファイルに情報を追記し、todoディレクトリに移動する（pending → todo）
...
6. レビュー待ちのタスクファイルを見つけたら下記を実施
    1. レビュー開始
        ...
        - 作業結果を確認する
            ...
            - 作業結果に問題があればレビュー指摘をタスクファイルに追記し、todoに戻す
    2. 作業結果に問題がなければタスクファイルをdoneに移動
    3. 監視に戻る
...
1. 引数で下記を受け取る。
    ...
    - エージェント名
2. membersディレクトリにファイル配置（多重起動チェック）
    - `DEV_{エージェント名}.md` が既に存在する場合は終了（同一Devの多重起動防止）
3. 対象ディレクトリを監視
    - 監視対象（いずれもプロジェクトルート直下）。複数のイベントを同時に検知した場合は、以下の優先順位で処理する。
        1. エージェント一覧置き場: `.my-workflow/members`（停止チェック最優先）
