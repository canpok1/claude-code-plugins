...
1. `gh issue list` でopen状態かつ `ready` ラベル付きのIssueを取得する。
2. 以下の条件に該当するIssueを除外する:
...
3. 除外対象のIssueから `ready` ラベルを除去し、除外理由をログに記録する。
4. 残りのIssueを `issue-assigner` エージェントの優先度基準に従って優先順位付けする。
5. 上位N件（`$ARGUMENTS` の `--count` で指定、デフォルト: 2件）に `assign-to-claude` ラベルを付与する。
