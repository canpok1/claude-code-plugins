...
- **正常停止** : PdMがmembersディレクトリから対象エージェントのファイルを削除する。スクリプトは次の監視サイクルで自身のファイルがないことを検知し終了する。
- **全体停止** : PdMがmembersディレクトリ内の全ファイルを削除する。
...
4. 対象ディレクトリとユーザー入力を監視
    - 監視対象（いずれもプロジェクトルート直下）
        ...
        - 検収待ちの仕様書置き場: `.my-workflow/work/designs/review`
...
6. 検収待ちの仕様書ファイルを見つけたら下記を実施
    ...
    2. 「ユーザー向けメッセージを表示」に戻り再開
7. ユーザーから入力があったら下記を実施
    1. 要望を詳細化
        - `claude "/request {ユーザーからの入力}"`
    2. 「ユーザー向けメッセージを表示」に戻り再開
8. エージェント一覧に自身のエージェント名のファイルが見つからない場合は下記を実施
    1. スクリプト終了
...
3. 対象ディレクトリを監視
    - 監視対象（いずれもプロジェクトルート直下）。複数のイベントを同時に検知した場合は、以下の優先順位で処理する。
        1. 回答済みの質問ファイル置き場: `.my-workflow/work/questions/answered`
        2. レビュー待ちのタスクファイル置き場: `.my-workflow/work/tasks/review`
        3. 未着手の仕様書置き場: `.my-workflow/work/designs/todo`
        4. エージェント一覧置き場: `.my-workflow/members`
...
3. 対象ディレクトリを監視
    - 監視対象（いずれもプロジェクトルート直下）
        ...
        - エージェント一覧置き場: `.my-workflow/members`
4. ファイル名に自身のエージェント名が含まれるタスクファイルを見つけたら下記を開始
